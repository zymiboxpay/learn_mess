!function(global, DOC) {
    ...
    var NsKey = DOC.URL.replace(rmakeid, ""); //长命名空间（字符串）//@zhu Ns 是 NameSpace 是意思
    var NsVal = global[NsKey]; //长命名空间（mass对象）//@zhu 第一次加载mass.js时 global[NsKey] 为 undefined ，搜索 NsKey 关键字，可以发现之后有 global[NsKey] = NsVal
    ...
    var mass = 1; //当前框架的版本号 //@zhu 这个变量之后会直接赋给 $.mass
    var postfix = ""; //用于强制别名
    ...
    //多版本共存
    if (typeof NsVal !== "function") { //@zhu 第一次加载 mass.js 时， NsVal 为 undefined ，否则为 function
        NsVal = $; //公用命名空间对象
        NsVal.uuid = 1;
    }
    if (NsVal.mass !== mass) { //@zhu 如果引用了多个版本的 mass 
        NsVal[mass] = $; //保存当前版本的命名空间对象到公用命名空间对象上 //@zhu 覆盖版本号为 function
        if (NsVal.mass || ($$ && $$.mass == null)) {
            postfix = (mass + "").replace(/\D/g, ""); //是否强制使用多库共存 //@zhu 替换非数字
        }
    } else {
        return;
    }
